...
def userkaihu():
    ...
    type = input("请选择您办理的银行卡类型，\n1.普通卡（每次最大转账额度为2万元）\n2.白金卡（每次最大转账额度为5万元）\n")
    if type == "1":
        type = "普通卡"
    elif type == "2":
        type = "白金卡"
    account = str(random.randint(10000000,99999999))
    fhz = bankkaihu(name,ID,password,country,province,street,House,money,account,type)
    if fhz == 1:
...
#银行开户
def bankkaihu(name,ID,password,country,province,street,House,money,account,type):
    con = pymysql.connect(host="localhost", user="root", password="123456", database="bank")
    # 2.创建控制台
    cursor = con.cursor()
    sql = "select count(*) from abcbank"#判断数据库是否大于100
    cursor.execute(sql)
    ...
    cursor.close()
    con.close()
    sql = "select ID from abcbank where 'money' >= %s"
    param = [0]
    data = select(sql, param, mode="all")
    for i in data:#返回字典里的所有键
        if ID in i:#如果ID在数据库里，返回2
            return 2
    sql = "insert into abcbank values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    param = [account,ID,name,password,country,province,street,House,money,type,bankname]
    update(sql,param)
    return 1
...
def cunqian():
    ...
    if bianhao == 1:
        ...
        #账号存在的情况下 存钱
        sql = "update abcbank set money = money + %s  where account = %s"
        param = [money,account]
        update(sql,param)
        #存款成功后查询余额
...
def bankqvkuan(account,password,money):
    ...
    if not data:#如果不存在返回1
        return 1
    #判断密码对不对
    sql = "select * from abcbank where password = %s and account = %s"
...
def qvqian():
    ...
    if bianhao == 1:
        ...
        update(sql,param)
        #取款成功，查询剩余余额
        sql = "select abcmoney from bank where account = %s"
        param = [account]
        data = select(sql,param,mode="all")
...
def chaxun():
    ...
    if bianhao == 1:
        ...
        --------------------------------------------
        '''
        print(info % (data[0][2],data[0][0],data[0][1],data[0][4],data[0][5],data[0][6],data[0][7],data[0][8],data[0][9],data[0][10]))
...
def bankzhuanzhang(account1,account2,password,money,lei):
    if lei == "1":
        ...
        if not data:  # 如果不在
            return 1
        # 判断密码对不对
        sql = "select * from abcbank where account = %s and password = %s"
        ...
        if account1 == account2:
            return 5
        #判断转出账户是普通用户还是白金用户
        sql = "select type from abcbank where account = %s"
        param = [account1]
        data = select(sql,param,mode="all")
        if data[0][0] == "普通卡":
            if money > 20000:
...
def zhuanzhang():
    ...
    if bianhao == 1:
        ...
        print("转账失败！尊敬的白金卡用户，每次最大转账额度为5万元，请您分次转账！")
    elif bianhao == 0:
        #进行转账,转出账户减掉金额
        if lei == "1":
            sql = "update abcbank set money = money - %s where account = %s"
            ...
            # 转入账户加金额
            print("转账中！！！！！！！！！！！！！！！！！！！！！")
            sql = "update bank set money = money + %s * 0.999 where account = %s "
            param = [money, account2]
            update(sql, param)
            # 转账成功，查询余额
            sql = "select money from abcbank where account = %s"
